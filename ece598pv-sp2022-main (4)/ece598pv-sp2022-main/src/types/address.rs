...
impl Address {
    pub fn from_public_key_bytes(bytes: &[u8]) -> Address {
        ...
        let last_bytes_slice: &[u8] = &digest_bytes[digest_bytes.len()-20 .. digest_bytes.len()];
        let last_bytes_arr: &[u8; 20] = last_bytes_slice.try_into().expect("wrong length"); // we do not expect the try_into to panic

        Address::from(last_bytes_arr)
    }
}
